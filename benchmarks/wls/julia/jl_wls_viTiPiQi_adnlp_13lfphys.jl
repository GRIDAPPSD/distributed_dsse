...
#function optimize()
  ...
  for row in CSV.File("test/measurement_data.csv")
    ...
    for i = 1:nnode
      println("$(nodeidx_nodename_map[i]) exp: $(target_solution[i]), sol: $(stats.solution[i]), diff: $(abs(target_solution[i]-stats.solution[i]))")
    end
    println("")
    for i = 1:nnode
      println("$(nodeidx_nodename_map[i]) exp: $(target_solution[i+nnode]), sol: $(stats.solution[i+nnode]), diff: $(abs(target_solution[i+nnode]-stats.solution[i+nnode]))")
    end
    println("")
